...
`AGENTS.md` and `README.md` are the only markdown at the repo root. This file
must stay here — Codex loads it from the root of the working tree, and
...
- **The fallback is never validated.** It is rendered `On "<claim>" — <base>`,
  ...
  construction. AGENTS.md rule 5 requires every LLM call to have a fallback, so
